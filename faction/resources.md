...
> *"Zeal wins the square. Bread holds it. Guess which one the poets sing about, and which one keeps me up at night."*
> — a quartermaster, to no one in particular
...
| **Trade actions** | Swap stock 1:1 with a faction at Friendly or better. Market dealing and off-the-books conversions are a [Spine](characters/classes/spine.md)'s personal trade, not a faction action. |
...
The [Keep](faction/faction-turn.md#2--keep) holds two kinds of need, and they fail differently:

- A **consumed need** is spent every turn — the garrison eats its Provisions, the presses burn their coal. Pay it and the stock is gone; miss it and hunger begins that turn.
- A **standing need** must merely be *present* — a garrison needs 1 Morale on hand to stay loyal, a shrine needs 1 Unity to stay holy. You never spend it, and for turn after turn it costs nothing and does nothing. It matters only the moment it's gone: spend that Morale on an edge, or let a rival steal it, and the standing need is unmet the instant the stock leaves. A quiet obligation, until it isn't.

## Going Hungry

When a need in the Keep goes unmet — a consumed need unpaid, or a standing need whose stock has left:
...
2. **Kept unmet:** the GM starts a visible [countdown](core/contests.md#countdowns), usually 3 segments — *The Kitchens Go Dark*, *The Garrison Wavers*. It ticks each further turn the need stays unmet. At zero, something is genuinely lost: people walk, a holding flips or empties, and [Cohesion](faction/overview.md#cohesion) drains 1 (2 for the Stronghold).

A standing need, once restored — the Morale replaced, the shrine re-sanctified — stops the bleeding at whatever step it reached. Neglect is a slow wound, and a reversible one, right up until it isn't.
